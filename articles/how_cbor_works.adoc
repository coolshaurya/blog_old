= A overview of CBOR's syntax
:toc:

Today, we will be looking at how https://cbor.io[CBOR]'s syntax is structured and how it encodes data. For the unfamiliar, CBOR is a data format that is inspired from JSON and whose data model heavily borrows from JSON, with the major difference being that CBOR is a binary data format while JSON is a text-based data format. This means that JSON is encoded in plaintext which most text editors can process easily, while CBOR is an arbitrary sequence of bytes which cannot be opened easily in text editors.

Usually, binary data formats can be decoded and encoded faster than text-based data formats, while being more compact than text-based data formats. However, text-based formats have the advantage that they are easier to read by a human as any text editor can open them and don't need custom decoders. The choice between a binary data format and a text-based one is a tradeoff between machine-readablility and human-readablility footnote:[Unless you're using YAML, which is terrible at both.].

== The data item

CBOR organizes data into _data items_. Each data item represents a single chunk of CBOR data. Data items can contain other data items nested inside them. Since CBOR is a binary data item, each data item is essentially some list of bytes (and not Unicode text
footnote:[Which link:https://en.wikipedia.org/wiki/Comparison_of_Unicode_encodings[can be encoded]
in either UTF-16 (mostly in Windows™ and Qt) or UTF-8 (more common) or UTF-32 (somewhat rare).], which would be the case in a text-based data format).
